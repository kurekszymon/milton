...
int main() {
  ...
  auto renderer = Renderer([&] {
    return vbox({text("Selected 1: " + std::to_string(selected_1)),
                 text("Selected 2: " + std::to_string(selected_2)),
                 text("Selected 3: " + std::to_string(selected_3)),
                 text("Selected 4: " + std::to_string(selected_4)),
                 text("Press arrow keys to change selections.")});
  });

  auto layout = Container::Vertical({Container::Horizontal({
